...
# アクセストークン有効期限 (1.0.1予定)
サーバー側でアクセストークンの有効期限を管理、チェック追加

# 設定情報のエクスポートとインポート (1.0.1予定)
ビルド時点で平文、暗号化の2通りから選択

# デシベル値範囲フィルター (1.0.1予定)
最小から最大値の範囲に絞り込み

# CSVデータダウンロード・アップロード機能 (1.0.1予定)
日時、デシベル値、GPS座標などのデータを  
CSV形式でダウンロード、アップロード(リスト表示反映)

# AndroidのOSSライセンス (1.0.1予定)
